...
const isNumber = value => +value - +value < 1;
...
describe('deleteNested', () => {
	...
	it('should not delete a nested array which has message prop', () => {
		const arr = [
			{
				c: 'foo',
			},
		];
		arr.message = 'some error';
		...
		const result = deleteNestedToRoot('a.b.0', obj);
		expect(result.a.b[0]).toEqual(undefined);
		expect(result.a.b.length).toEqual(1);
	});
});
...
describe('swap', () => {
	...
	it('should handle sparse arrays', () => {
		const arr = [1, 2, 3];
		arr[4] = 4;
		const expected = [1, 4, 3, undefined, 2];
		const result = swap(arr, 1, 4);
		expect(result).toEqual(expected);
	});

	it('should handle sparse arrays (big gap)', () => {
		const arr = [1, 2, 3];
		arr[7] = 4;
		const expected = [1, 2, 4, undefined, undefined, undefined, undefined, 3];
...
describe('insert', () => {
	it('should insert an element at the specified index', () => {
		const arr = [1, 2, 3, 4];
		const result = insert(arr, 2, 'new');
		expect(result).to.deep.equal([1, 2, 'new', 3, 4]);
	});

	it('should handle sparse arrays', () => {
		const arr = [1, 2, 3];
		arr[5] = 6;
		const result = insert(arr, 4, 'new');
		expect(result).to.deep.equal([1, 2, 3, undefined, 'new', undefined, 6]);
	});

	it('should return a new array without modifying the original array', () => {
		const arr = [1, 2, 3];
		const result = insert(arr, 1, 'new');
...
describe('getErrorClassName', () => {
	it('should return a string with the classNameError and the error type', () => {
		...
		const result = getErrorClassName(className, classNameError, errorObject);
